...
export const fetchConnectionEvents = async (id) => {
    try {
        ...
        const connectionEvents = events.filter((event) => connection?.connections.includes(String(event.id)));
...
export const fetchConnectionGroups = async (id) => {
    try {
        ...
        const connectionGroups = groups.filter((group) => connection?.groups.includes(String(group.id)));
...
export const fetchConnectionConnections = async (id) => {
    try {
        ...
        const connectionConnections = users.filter((user) => connection?.connections.includes(String(user.id)));
