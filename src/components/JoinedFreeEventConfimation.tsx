...
const JoinedFreeEventConfimation = () => {
  ...
  useEffect(() => {
    if (event?.id && !isJoining && uiState !== "joined") {
      const checkAttending = async () => {
        try {
          const attending = await isUserEventAttendee(event.id);
          console.log("isUserEventAttendee result:", attending);
          setUiState(attending ? "joined" : "idle");
        } catch (error) {
  ...
  }, [event, isUserEventAttendee, isJoining]);

  useEffect(() => {
    console.log("uiState changed:", uiState);
  }, [uiState]);
  ...
  async function handleJoinEvent() {
    console.log("handleJoinEvent called, event.id:", event?.id);
    if (!event?.id) {
    ...
    try {
      ...
      setUiState("joined");
      console.log("Set uiState to joined");
    } catch (error) {
...
export default JoinedFreeEventConfimation;
