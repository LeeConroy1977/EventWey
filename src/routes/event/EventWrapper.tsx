...
const EventWrapper: React.FC<EventWrapperProps> = ({
  ...
  try {
    const eventDate =
      typeof date === "number" ? new Date(date) : new Date(parseInt(date, 10));
  ...
  const handleCancelFreeAttendance = () =>
    openEventModal(event, "cancelFreeEvent");
      ...
      <div className="w-full tablet:w-[94%] desktop:w-[66%] h-[100%] flex flex-col tablet:flex-row items-center justify-center mt-0 tablet:mt-6">
        ...
        <div className="w-full tablet:w-[50%] h-[100%] flex flex-col items-center justify-start pl-0 p-0 tablet:pl-12 desktop:pl-16 tablet:p-2">
          ...
          <div className="w-full flex items-center mt-6 desktop:mt-auto desktop:mr-auto  ">
            <IoPerson className="text-primary text-[16px] xl-screen:text-[18px]" />
          ...
          {!isMobile && approved && (
            <button
              onClick={
                isAttending && free
                  ? handleCancelFreeAttendance
                  : isAttending && !free
                  ? handleCancePaidlAttendance
                  : !isAttending && free
                  ? handleJoinEvent
                  : handleGetTickets
              }
